...
def callback(data):
    ...
    """new_od.pose.pose.position.x=pose[0]
    new_od.pose.pose.position.y=pose[1]
    new_od.pose.
    pub.publish(new_od)
    """
if __name__ == '__main__':
    global new_od, pub, sub
    ...
    # get odom topic
    if rospy.has_param("/p3dx/odom"):
        odom_topic = rospy.get_param("/p3dx/odom")
    else:
        odom_topic = "/odom"
    # get base frame
    ...
    # get base frame
    if rospy.has_param("p3dx_tf/odom"):
        odom_frame = rospy.get_param("p3dx_tf/odom")
    else:
        odom_frame = "odom"
 
    rospy.Subscriber(odom_topic, Odometry, callback)
    #pub = rospy.Publisher("/p3dx/new_od", Odometry, queue_size=2)

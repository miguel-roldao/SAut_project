...
def new_update(pose, cov, landmark_coord, matriz_covL):
...
def new_Mahalanobis_recognition(state_vector, matriz_covL, measurementBar):
    ...
    for i in range(0, measurementBar.size, 2):
        ...
        mahalanobis_D[N] = 0.005 #parametro
...
if __name__ == '__main__':
	...
	while threshold < 1655997355.0:

		pose, cov, X, Y, threshold = import_match_data("./lab_SA/prediction_data.txt", "./lab_SA/laser_data.txt",threshold)
		...
		state_vector, matriz_covL = new_update(pose,cov,landmark_coord, matriz_covL)

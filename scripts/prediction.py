...
def simple_prediction_callback(msg_toreceive: Odometry):
    global positionx, positiony, yaw,iniciado,matriz_cov,t0, it
    ...
    if it%10 == 0:
        if(iniciado!=0):
            ...
            cov = np.concatenate( [matriz_cov.flatten(),np.zeros(27)])
            ...
            with open("./lab_SA/prediction_data.txt","a") as f1:
                f1.write("time: "+str(t2)+"\n")
                f1.write(str(positionx)+"\t"+str(positiony)+"\t"+str(yaw)+"\n")
                for i in cov:
                    f1.write(str(i)+"\t")
...
if __name__== '__main__':
    global positionx, positiony, yaw,matriz_cov, t0, it
    ...
    f1 = open("./lab_SA/prediction_data.txt","w") #clear file
    f1.close()
    ...
    sub_odometry=rospy.Subscriber("/pose", Odometry, callback = simple_prediction_callback) #callback means it call a function when it receives information


    pub=rospy.Publisher("/prediction_calculation", PoseWithCovariance, queue_size=10)

    #----------------------

    #sub_pose=rospy.Subscriber("/pioneer/pose_belief", PoseWithCovariance)
        
    #sub_odometry=rospy.Subscriber("/p3dx/odom", Odometry, callback = simple_prediction_callback)#callback means it call a function when it receives information
